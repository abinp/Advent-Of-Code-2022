...
function totalScore() {
  ...
  roundStrategyList.forEach((currentRound) => {
    // const currentRound = roundStrategyList[0];
    // currentRound
    const currentRoundScore = updatedRoundScore(currentRound);
    // console.log(currentRound, currentRoundScore)
    totalScore = totalScore + currentRoundScore;
  });
  console.log(totalScore);
}

function roundScore({ opponent, player }: {opponent: string, player: string }): number {
  const dictionary = {
    'X': 'Rock',
    'Y': 'Paper',
    'Z': 'Scissor',
...
function updatedRoundScore({ opponent, player }: {opponent: string, player: string }): number {
  const dictionary = {
    'X': 'Rock',
    'Y': 'Paper',
    'Z': 'Scissor',
  ...
  if(player == 'X') {
    console.logdefaultScore[resultMatrix[opponent][player]];
  }
  if(player == 'Y') {
