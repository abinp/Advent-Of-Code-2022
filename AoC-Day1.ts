...
async function maxCalories() {
  ...
  elfsByCalories.forEach((foodWithEachElf: string, index: number) => {
    const calorieList = foodWithEachElf.split('\n').map((calorie: string) => Number(calorie));
    const totalCalories = calorieList.reduce((total: number, currentCalorie: number) => currentCalorie + total, 0);
...
async function topThreeMaxCalories() {
  ...
  console.log({ maxCalories1, maxCalories2, maxCalories3 }, maxCalories1 + maxCalories2 + maxCalories3);
}
